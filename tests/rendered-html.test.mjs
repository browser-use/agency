...
test("keeps card decisions fast and actionable", async () => {
  ...
  assert.match(ui, /taskSubmitting \? "Sending…" : "Send"/);
  assert.match(ui, /await submitNewTask\(task\)/);
  assert.match(ui, /taskSubmittingRef\.current\) return/);
  assert.match(ui, /composerError && <p className="radar-task-error" role="alert"/);
  assert.match(ui, /Task queued · #\$\{jobId\}/);
  ...
  assert.match(ui, /selectIdea\(keepSelectedCard\(anchor, visible, next\.ideas\)\)/);
  ...
  assert.match(ui, /Nothing sent\. Check the current card and try again\. Your draft is saved/);
  assert.doesNotMatch(ui, /This card changed in the background|Show update|showLatestSelected|radar-update-waiting/);
  ...
  assert.match(ui, /\[actionable, idea\.id, idea\.cardHtml, idea\.jobOutcome\]/);
  assert.match(ui, /needsFallbackAction\(missingDo, explicitlyBlocked, idea\.jobOutcome\)/);
  ...
  assert.match(ingest, /A suggestion needs a meaningful next-step button/);
  assert.match(ingest, /cardIngestMode\(cardHtml, card\.blockedJobId, card\.expectedVersion\)/);
